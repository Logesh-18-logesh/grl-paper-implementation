...
class SplitterTrainer(object):
    ...
    def save_base_node_embedding(self):
        """
        Save the base node embedding before persona splitting.
        """
        print("\nSaving base node embeddings...\n")
        nodes = list(self.graph.nodes())
        embedding = self.base_node_embedding
        if isinstance(embedding, np.ndarray):  # Already numpy
            pass
        else:
            embedding = np.array(embedding)
        df = pd.DataFrame(embedding)
        df.insert(0, "id", nodes)
        df.to_csv("./output/chameleon_base_embedding.csv", index=False)
